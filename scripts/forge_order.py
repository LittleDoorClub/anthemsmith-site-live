...
import json, os, re, sys, time, hashlib, subprocess, threading, urllib.request, urllib.error
...
# die()/reject_unpaid() entirely: no songs/<OID>.failed.json, no song, no delivery
# -- the only trace is a `cancelled` run whose duration equals the cap (measured on
# PAID order AS-MUHOETYD: 20m17s then 20m16s). A deadline INSIDE the cap guarantees
# a failure record and a bounded run no matter which step hangs (IG scrape / poll / CDN).
...
if IG_URL:
    ...
    if ig_ingest is not None:
        ...
        if hasattr(ig_ingest, "scrape_instaloader"):
            # A hung instaloader 429-retry loop must NEVER block the forge.
            # `ThreadPoolExecutor.__exit__` calls shutdown(wait=True) BEFORE the
            # except handler, so result(timeout=60) could not BOUND the run (the
            # handler + Chrome fallback ran only after the worker finished). Live:
            # PAID order AS-MUHOETYD logged "429 Too Many Requests [retrying]" at
            # 01:54:40Z and 02:05:47Z, then "##[error]The operation was canceled."
            # at ~20m17s, with songs/<OID>.json 404 AND songs/<OID>.failed.json 404.
...
pre2 = clean("Steady hands, steady heart, never missed a single inch,")
...
# DURATION LAW (180 s <= duration <= 225 s): with V6 retired there is NO `duration`
# field, and song LENGTH tracks the LYRIC STRUCTURE. Measured 2026-09-26 on this exact
# style: a 5-section / 1,185-char lyric returned 141.6 s / 129.6 s -- BOTH below the
# 180 s floor -- while this 9-section / ~2,070-char form returned 193.6 s / 193.2 s,
# both IN BAND. Do not shorten this template without re-measuring.
lyrics = f"""[Intro]
{v2}
...
Every little detail a signature, every quiet thing a star,
...
def poll(rid, max_s=300):
    # max_s is sized so ONE model attempt fits well inside the job's
    # `timeout-minutes: 20`; the model chain calls this per model, so the SUM of
    # windows must stay under the cap (a job-level cancel bypasses die() and leaves
    # a paid order with NO failure record at all).
    t0 = time.time()
    while time.time() - t0 < max_s:
        ...
        try:
            ...
            # A 4xx here is a TERMINAL task failure (e.g. 400 "Invalid mv"), not
            # "still running". The old `except Exception: continue` swallowed it and
            # slept out the whole window (measured: 28 min of silence -> die()).
            try:
                body = he.read().decode("utf-8", "replace")[:300]
            except Exception:
                body = ""
            print("  poll HTTP %s (terminal): %s" % (he.code, body), flush=True)
            return {"status": "failed", "error": "HTTP %s: %s" % (he.code, body)}
        except Exception as e:
            print("  poll transient: %r" % (e,), flush=True)
            continue
        st = d.get("status")
        if st in ("completed", "succeeded", "success"): return d
        if st in ("failed", "error"):
            print("  poll status=%s error=%s" % (st, str(d.get("error"))[:200]), flush=True)
            return d
    return {"status": "timeout"}

gender = "male" if VOICE.lower().startswith("m") else ("female" if VOICE.lower().startswith("f") else "male")
# MODEL CHAIN (2026-09-26). V6 is RETIRED on MuAPI: a V6 submission is accepted
# (request_id + cost) and then the result endpoint returns HTTP 400
#   {"status":"failed","error":"Invalid mv. Supported values: V4, V4_5, V4_5ALL, V4_5PLUS, V5, V5_5"}
# so submitting V6 first made generation dead for EVERY real order. No live model
# accepts `duration`, so length comes from the 9-section lyric above.


def _submit(model):
    return post({"prompt": lyrics, "style": style, "title": f"AnthemSmith {OID}", "custom_mode": True,
                 "instrumental": False, "vocal_gender": gender, "model": model,
                 "negative_tags": "sad, ballad, piano lead, cheesy, autotune"})


outs = []
model_used = None
for _m in ("V4_5", "V4_5PLUS", "V5_5"):
    sub = _submit(_m)
    rid = sub.get("request_id") or sub.get("requestId") or (sub.get("data") or {}).get("request_id")
    print("SUBMIT model=%s rid=%s" % (_m, rid), flush=True)
    if not rid:
        continue
    res = poll(rid)
    ...
    if outs:
        model_used = _m
        break
    print("  no outputs (status=%s) -> next model" % (d.get("status"),), flush=True)
if not outs:
