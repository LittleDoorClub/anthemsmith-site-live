...
function watchOrder(id,box,tries){
 ...
 let n=0;const timer=setInterval(async()=>{
  ...
  try{
   ...
   if(rd&&rd.ok){const dd=await rd.json().catch(()=>null);if(dd&&dd.delivery_status==='delivered'){deliveryNote=' <span class="delivery-ok">\u2713 Delivered to your '+(dd.mode==='sms'?'phone':'email')+'</span>';}
    else if(dd&&dd.delivery_status==='submitted'){deliveryNote=' <span class="delivery-ok">\u2713 Sent to your '+(dd.mode==='sms'?'phone':'email')+'</span>';}
...
function resumeAfterPayment(){
 const raw=sessionStorage.getItem('anthemsmith_order')||(new URLSearchParams(location.search).get('oid')?JSON.stringify({request_id:new URLSearchParams(location.search).get('oid')}):null);if(!raw)return false;
